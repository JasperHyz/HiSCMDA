...
input_file = 'mddd_motif_SEED_{0}_FOLD_{1}'.format(SEED, FOLD)
output_file = 'mddd_step1_SEED_{0}_FOLD_{1}'.format(SEED, FOLD)
...
if tensor_max_index_search_modify(tensor_temp, id_dic):
    ...
    for i in range(0, len(tensor_temp)):
        # dddm
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][0],
                                                                                      tensor_temp[i][1],
                                                                                      tensor_temp[i][2], 
                                                                                      tensor_temp[i][3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][0],
                                                                                      ...
                                                                                      tensor_temp[i][1], 
                                                                                      tensor_temp[i][3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][1],
                                                                                      tensor_temp[i][0],
                                                                                      tensor_temp[i][2], 
                                                                                      tensor_temp[i][3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][1],
                                                                                      ...
                                                                                      tensor_temp[i][0], 
                                                                                      tensor_temp[i][3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][2],
                                                                                      tensor_temp[i][0],
                                                                                      tensor_temp[i][1], 
                                                                                      tensor_temp[i][3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(tensor_temp[i][2],
    ...
    for i in range(0, len(tensor_origin)):
        ...
        # dddm
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[0],
                                                                                      mapped_tensor[1],
                                                                                      mapped_tensor[2], 
                                                                                      mapped_tensor[3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[0],
                                                                                      ...
                                                                                      mapped_tensor[1], 
                                                                                      mapped_tensor[3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[1],
                                                                                      mapped_tensor[0],
                                                                                      mapped_tensor[2], 
                                                                                      mapped_tensor[3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[1],
                                                                                      ...
                                                                                      mapped_tensor[0], 
                                                                                      mapped_tensor[3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[2],
                                                                                      mapped_tensor[0],
                                                                                      mapped_tensor[1], 
                                                                                      mapped_tensor[3], 1)
        content += ("" if len(content) == 0 else "\n") + '{0} {1} {2} {3} {4}'.format(mapped_tensor[2],
